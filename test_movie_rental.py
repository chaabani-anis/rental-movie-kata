from movie_rental import Customer, Rental, Movie, Render, RenderText
...
def tests():
    ...
    assert expected == customer.statement(RenderText())
def tests():
    customer = Customer("Bob")
    customer.addRental(Rental(Movie("Jaws", Movie.REGULAR), 2))
    customer.addRental(Rental(Movie("Golden Eye", Movie.REGULAR), 3))
    ...
    assert expected == customer.statement(RenderHtml())
